"""Rebuild the four-model train/test ROC figure from the archived package.

Inputs (all shipped with the package):
    data/training_points_70ha_seed42.csv          (from data.zip)
    models/<MODEL>/thr70/seed42/final_model.keras (from models.zip)
                              /static_preprocessor.joblib
                              /selected_features_final.csv
                              /metrics_summary.json
Output:
    figs/Fig_ROC_4models_train_test.png / .pdf

Run from the package root, inside the `wildfire` conda environment:
    python make_roc_figure.py
    python make_roc_figure.py --check   (verify inputs only, no TensorFlow needed)

The 85/15 stratified split uses the fixed seed 42, so the curves and AUC
...
CSV    = os.path.join(HERE, "data", "training_points_70ha_seed42.csv")
MDLDIR = os.path.join(HERE, "models")
...
NEEDED = ("final_model.keras", "static_preprocessor.joblib",
          "selected_features_final.csv", "metrics_summary.json")


def check_inputs():
    ok = True
    if os.path.exists(CSV):
        print(f"[OK]   {os.path.relpath(CSV, HERE)}")
    else:
        print(f"[MISS] {os.path.relpath(CSV, HERE)}  -> unpack data.zip into data/")
        ok = False
    for k, info in MODELS.items():
        d = os.path.join(MDLDIR, info["dir"], "thr70", "seed42")
        for n in NEEDED:
            p = os.path.join(d, n)
            if os.path.exists(p):
                print(f"[OK]   {os.path.relpath(p, HERE)}")
            else:
                print(f"[MISS] {os.path.relpath(p, HERE)}  -> unpack models.zip into models/")
                ok = False
    return ok


if "--check" in sys.argv:
    sys.exit(0 if check_inputs() else 1)

if not check_inputs():
    sys.exit("Missing inputs - see [MISS] lines above.")
...
def model_predict(model_key):
    ...
    m_dir = os.path.join(MDLDIR, info["dir"], "thr70", "seed42")
...
for k in MODELS.keys():
    ...
    if out is not None:
        ...
        ms_path = os.path.join(MDLDIR, MODELS[k]["dir"], "thr70", "seed42", "metrics_summary.json")
...
plt.tight_layout()
pdf_out = os.path.join(FIGDIR, "Fig_ROC_4models_train_test.pdf")
png_out = os.path.join(FIGDIR, "Fig_ROC_4models_train_test.png")
