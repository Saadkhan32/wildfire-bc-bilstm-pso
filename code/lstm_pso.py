import os, math, json, argparse, warnings, time, random
...
try:
    register_ks = K.saving.register_keras_serializable
except Exception:
...
def build_lstm_cnn(n_features: int,
                   conv_filters: int,
                   kernel_size: int,
                   units1: int,
                   units2: int,
    ...
    x = K.layers.LSTM(int(units1), return_sequences=(int(units2) > 0))(x)
    if int(units2) > 0:
        x = K.layers.LSTM(int(units2))(x)
    x = K.layers.Dropout(float(dropout))(x)
    x = K.layers.Dense(64, activation="relu")(x)
    out = K.layers.Dense(1, activation="sigmoid")(x)
    model = K.Model(inp, out, name="lstm_cnn")
    model.compile(
        optimizer=opt,
...
def make_pso_objective(X_df: pd.DataFrame, y: np.ndarray, groups: np.ndarray,
    ...
    def decode(vec):
        ...
        bs = int(2 ** int(np.clip(round(vec[6]), 5, 7)))
    ...
    def objective_fn(particles):
        ...
        for p in tqdm(particles, desc="PSO particle evals", leave=False):
            ...
            losses.append(1.0 - float(score))
        return np.array(losses, dtype=np.float64)
    return objective_fn
def main():
    parser = argparse.ArgumentParser(description="LSTM + PSO + RFE (spatial CV, compact search, Spyder-friendly)")
    parser.add_argument("--data", type=str, default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "training_points_70ha_seed42.csv"))
    parser.add_argument("--out_dir", type=str, default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "outputs", "lstm_pso"))
