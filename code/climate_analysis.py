...
COMPOSITE_GROUPS = [
    ...
    ("atmospheric",  ["specific_humidity","avg_wind_speed", "PDSI"]),
]
...
DEFAULT_MERGED = os.path.join(HERE, "..", "data", "climate",
                              "merged_monthly_climate_2000_2024.csv")
def parse_args():
    ap = argparse.ArgumentParser(
        description="Seasonal climate comparison: wildfire vs non-wildfire")
    ap.add_argument("--climate-csv", default=DEFAULT_MERGED,
                    help="merged monthly climate CSV "
                         "(default: shipped data/climate/merged_monthly_climate_2000_2024.csv)")
    ap.add_argument("--raw", nargs="+", default=None,
                    help="raw TerraClimate CSVs + wide climate CSV, used instead of --climate-csv")
    ap.add_argument("--root", default=os.path.join(HERE, ".."),
                    help="project root for outputs (writes tables/ and figs/; default: repo root)")
    ...
    for i, season in enumerate(["Non-Wildfire Season", "Wildfire Season"]):
        ...
        if not np.isnan(m):
            ax_box.errorbar(i + 0.18, m, yerr=hw, fmt="o", color="black",
                            ...
                            zorder=10)
            ax_box.annotate((f"{m:.4f}" if abs(m) < 0.1 else (f"{m:.2f}" if small else f"{m:.3f}")),
                            (i + 0.18, m), xytext=(6, 6),
...
def write_publication_table(stats_df, out_tables):
    ...
    caption = (
        ...
        "specific humidity from ERA5-Land reanalysis; "
        ""
        "PDSI, AET, and soil moisture from TerraClimate. "
...
def main():
    print("=" * 75)
    print(" Climate Stats - FULL battery (monthly-aggregate)")
    print(f" Started: {datetime.now():%Y-%m-%d %H:%M:%S}")
    print("=" * 75)
    args = parse_args()
    if args.raw:
        df = remerge([Path(p) for p in args.raw])
        print(f" re-merged {len(args.raw)} files: shape {df.shape}")
    else:
        df = pd.read_csv(args.climate_csv)
        df = df.rename(columns=VARIABLE_RENAME)
        print(f" loaded merged: {os.path.basename(args.climate_csv)}  ({df.shape[0]} rows x {df.shape[1]} cols)")
