...
try:
    ...
    sys.exit("Missing package: {0}\nInstall into the env you ran your last script "
             ...
             "matplotlib".format(e))
REPO = r"."
DEF_US_BP    = os.path.join(REPO, "data", "cross_border_US", "BP_WA_ID_MT.tif")
DEF_BC_STRIP = os.path.join(REPO, "data", "cross_border_US", "bc_strip.shp")
DEF_US_STRIP = os.path.join(REPO, "data", "cross_border_US", "us_strip.shp")
DEF_BORDER   = os.path.join(REPO, "data", "cross_border_US", "bc_us_border.shp")
FIG_DIR = os.path.join(REPO, "figs")
TAB_DIR = os.path.join(REPO, "data", "cross_border_US")
TARGET_EPSG = 3005
CELL        = 1500
N_BINS      = 5
MIN_CELLS   = 3
...
def main():
    ...
    args = ap.parse_args()
    log("=" * 64)
    log("  Cross-border: Tier 1 (consistency) + Tier 3 (edge)")
    log("=" * 64)
    ...
    if primary_pairs is not None:
        csv_path = os.path.join(TAB_DIR, "Fig19b_tile_pairs.csv")
    ...
    for ext in ("pdf", "png"):
        ...
        log("  wrote " + fp)
    plt.close(fig)
    json_path = os.path.join(TAB_DIR, "Fig19c_Table8_crossborder_stats.json")
