...
def ask_paths() -> Tuple[str, str]:
    ...
    ap = argparse.ArgumentParser(description="Train LSTM/BiLSTM with spatial cross-validation")
    ap.add_argument("--data", default=os.path.join(here, "..", "data", "training_points_70ha_seed42.csv"),
                    help="training points CSV/XLSX (default: shipped seed-42 table)")
    ap.add_argument("--out_dir", default=os.path.join(here, "..", "outputs", "lstm_bilstm_spatialcv"),
                    help="output folder (default: repo outputs/)")
    a = ap.parse_args()
    if not os.path.exists(a.data): raise SystemExit(f"Data file not found: {a.data}")
    os.makedirs(a.out_dir, exist_ok=True)
    return a.data, a.out_dir
