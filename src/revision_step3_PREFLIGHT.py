# -*- coding: utf-8 -*-
"""
revision_step3_PREFLIGHT.py
===========================
PRE-FLIGHT TEST for STEP 3 -- runs in VS Code or any IDE.

What it does:
  - Runs ONLY the first combo (thr70, seed42)
  - Tiny PSO budget (2 particles, 1 iter, 3 epochs) -> finishes in ~5-15 min
  - Writes into 05_Model_Results_PREFLIGHT/  (separate from real results)
  - LIVE PROGRESS BAR via tqdm
  - LIVE STREAMING of each model's stdout to the IDE terminal
    (you SEE epoch-by-epoch progress instead of staring at nothing)

VS Code usage:
  - Open the repo as a folder in VS Code
  - Open this file
  - Make sure your interpreter is set to the 'wildfire' conda env
    (Ctrl+Shift+P -> "Python: Select Interpreter" -> wildfire)
  - Press F5 (Run with debugger) OR Ctrl+F5 (Run without debugger)
  - The Tkinter dialogs pop up the same way

CLI usage (Anaconda Prompt) still works too:
    conda activate wildfire
    python src\revision_step3_PREFLIGHT.py
"""
import os
import sys

# ---- Unbuffered output so VS Code's Run terminal updates LIVE ----
...
# ============================================================
# PRE-FLIGHT settings (tiny so it finishes fast)
# ============================================================
PREFLIGHT_THR  = 70
PREFLIGHT_SEED = 42
...
# Where to cache the 6 paths so re-runs don't re-ask
...
# ============================================================
# Streaming subprocess runner with live tqdm postfix
# ============================================================
def run_streaming(cmd, log_path, pbar=None, label=""):
    """
    Run cmd; write every stdout line to log_path AND echo to terminal so
    you see the model's own progress in real time. Also updates the tqdm
    bar's postfix with the most recent informative line.
    """
    t0 = time.time()
    last_useful = ""
    with open(log_path, "w", encoding="utf-8") as logf:
        ...
        for line in proc.stdout:
            ...
            # Echo every line (indent for readability)
            tqdm.write(f"      [{label}] {stripped}")
            # Update progress bar postfix with informative lines
            low = stripped.lower()
...
# ============================================================
# Run the 4 models with a tqdm progress bar
# ============================================================
results = {}
t_total = time.time()
...
tasks = [
    ...
    ("nonPSO",     "nonpso", nonpso_cli),  # produces both LSTM and BiLSTM
]
...
for name, kind, script in tasks:
    ...
    else:  # nonpso CLI -> trains LSTM + BiLSTM
...
if all_ok:
    ...
    print("the error lines to Claude.", flush=True)
    print("=" * 70, flush=True)
