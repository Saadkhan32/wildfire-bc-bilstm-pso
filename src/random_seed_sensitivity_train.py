# -*- coding: utf-8 -*-
"""
random_seed_sensitivity_train.py
================================
Reviewer Comment 8 deliverable: random-seed sensitivity of the wildfire
LSTM / BiLSTM models using the 10 pseudo-absence datasets produced by
src/generate_pseudo_absence_seed_sets.py.

Design:
    - 1,992 BC-agency wildfire points + 1,992 pseudo-absence points per seed
    - 70/30 stratified train/test split per seed
    - Same preprocessing, RF-RFE, model architecture for every seed
    - Reports AUC, AP, Accuracy, F1, Precision, Sensitivity, Specificity, NPV
    - Bit-level reproducibility via enable_op_determinism (added)
    - Per-seed bootstrap CIs on AUC (added)
    - Cross-seed mean +/- SD summary at the end

Run in your wildfire conda env after generating the seed CSVs:
    python src/random_seed_sensitivity_train.py
"""
import os
import json
import random
...
# Hyperparameters matching the non-PSO production run (used for sensitivity test).
# If you also want to test PSO architecture per seed, swap these for the best_params
# emitted by your PSO training script.
BASE_HPS = {
...
BOOTSTRAP_N    = 1000   # bootstrap resamples for per-seed AUC CI
BOOTSTRAP_SEED = 42
...
def set_all_seeds(seed: int):
    """Pin every RNG and enable bit-level deterministic ops."""
    random.seed(seed)
    np.random.seed(seed)
...
def get_feature_names(pre: ColumnTransformer, X_df: pd.DataFrame) -> np.ndarray:
    """Robust feature-name extraction; uses 1-row sample (NOT empty frame) as fallback."""
    try:
...
def train_one_seed(seed, data_path, out_root) -> list:
    ...
    # Exact-match drop list (not substring) to avoid accidental drops of similarly-named columns
...
# ============================================================
# MAIN: loop over 10 seeds + summary
# ============================================================
def main():
