# -*- coding: utf-8 -*-
"""
c8c11_step3_run_all_models_sensitivity.py
=========================================
STEP 3 of 5 -- Option C (all 4 models).

What this does:
  Runs all 4 architectures on each of the 30 training CSVs from STEP 2:
      BiLSTM-PSO  (via BiLSTM PSO FE.py)
      LSTM-PSO    (via LSTM PSO FE.py)
      BiLSTM      (no PSO; via c8c11_non_pso_cli.py)
      LSTM        (no PSO; via c8c11_non_pso_cli.py)

  Layout of outputs:
      05_Model_Results/<MODEL>/thr<H>/seed<S>/metrics_summary.json
      (one folder per (model, threshold, seed) combination)

  Total runs:
      2 PSO models  x 10 seeds x 3 thresholds = 60 PSO calls
      1 non-PSO CLI x 10 seeds x 3 thresholds = 30 non-PSO calls
        (each non-PSO call trains BOTH LSTM + BiLSTM)

  Resumable: re-run anytime; completed (model, threshold, seed) folders
  are skipped.

Run:
    conda activate wildfire
    python src/c8c11_step3_run_all_models_sensitivity.py
"""
import os
import sys
import time
...
# PSO budget (matches your published runs)
...
for i, (thr, s, csv) in enumerate(plan, 1):
    ...
    # ---- non-PSO (LSTM + BiLSTM in one call) ----
