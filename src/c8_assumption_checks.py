# -*- coding: utf-8 -*-
"""
c8_assumption_checks.py
=======================
Reviewer Comment 8 sub-question 5: "assumptions met and tested?"

Runs three checks on the seed-42 outputs of random_seed_sensitivity_train.py:
    1. VIF on the RFE-selected predictors -> tables/T_vif_predictors.csv
    2. Probability calibration (reliability diagram + Brier) -> figs/Fig_S_calibration.png + tables/T_calibration_bins.csv
    3. Residual Moran's I (spatial independence of OOF errors) -> tables/T_residual_morans_i.csv

Run in your wildfire conda env after seed-42 results are produced.
    pip install statsmodels libpysal esda matplotlib scikit-learn

Usage:
    python src/c8_assumption_checks.py
"""
import os
import json
import numpy as np
...
# ============================================================
# USER PATHS (edit if your output locations differ)
# ============================================================
...
SEED_FOR_CHECKS   = 42       # which seed's outputs to audit
MODEL_FOR_CHECKS  = "BILSTM"  # "BILSTM" or "LSTM"
...
SEED_CSV       = SEED_FILES_FOLDER / f"training_seed_{SEED_FOR_CHECKS}.csv"
SEED_MODEL_DIR = OUT_ROOT / f"seed_{SEED_FOR_CHECKS}" / MODEL_FOR_CHECKS
...
# ============================================================
# Check 1: VIF on the RFE-selected predictors
# ============================================================
...
# RFE feature names are in the post-preprocessor transformed space (e.g. "num__NDVI",
# "cat__LULC_3"). Strip the pipeline prefix and map back to raw input columns where possible.
...
for s in selected:
    base = strip_prefix(s)
    # OneHot expands lulc into lulc_3, lulc_5, ... -> keep base
    base = base.split("_")[0] if base.lower().startswith("lulc") else base
    selected_raw.append(base)
selected_raw = list(dict.fromkeys(selected_raw))  # dedupe preserving order
...
# Use numeric features only (drop categorical like LULC if it survived)
...
# Standardise so VIF is comparable across scales
...
# ============================================================
# Check 2: Probability calibration (reliability diagram + Brier)
# ============================================================
print("\n========== Check 2: Calibration ==========")
...
# ============================================================
# Check 3: Residual Moran's I (spatial independence of OOF errors)
# ============================================================
...
# Match holdout predictions back to lat/lon via row index.
# The holdout_predictions.csv preserves the row order of the test split,
# so we use the same train_test_split seed to recover the lat/lon for those rows.
from sklearn.model_selection import train_test_split

# Reload the full seed CSV
...
# Re-run the same 70/30 split with seed = SEED_FOR_CHECKS
...
# One JSON for the rebuttal letter to pick up
summary = {
