# -*- coding: utf-8 -*-
"""
revision_step1b_copy_inputs.py
==============================
STEP 1b -- Tkinter-driven copy of all input files into revision_c8c11/.

Replaces the manual "drag files into the right folders" step. Pickers ask
you for each input once and the script copies + renames automatically.

Run:
    python src/revision_step1b_copy_inputs.py
"""
import os
import sys
import shutil
...
# Required raster filenames (in destination form)
REQUIRED_RASTERS = [
...
def copy_shapefile(src_shp, dst_folder, dst_name=None):
    """Copy a shapefile and all its siblings (.dbf, .shx, .prj, etc.)."""
    ...
    for ext in SHP_SIBLINGS:
        ...
        if os.path.exists(s):
            ...
            shutil.copy2(s, d)
            n_copied += 1
    # Also handle .shp.xml which has the funny double-extension
    sxml = base + ".shp.xml"
...
# 1. Pick the repo root (or revision_c8c11 base)
...
# 4. Predictor rasters: pick a folder, auto-match, then ask for any missing
...
# Build a lookup of (lowercase basename -> full path) in the source folder
src_files_lc = {}
for fn in os.listdir(src_rasters_dir):
    if fn.lower().endswith(".tif"):
        src_files_lc[fn.lower()] = os.path.join(src_rasters_dir, fn)

matched = {}      # required_name -> source_path
to_pick = []      # required_name list still needing manual pick
...
# Ask for each missing raster individually
for req in to_pick:
...
for req, src in matched.items():
    ...
    # Also copy any sidecar files (.tfw, .xml, .ovr, .aux.xml)
