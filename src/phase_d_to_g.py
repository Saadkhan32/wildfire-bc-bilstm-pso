#!/usr/bin/env python
# -----------------------------------------------------------------------------
# phase_d_to_g.py  -  Wildfire BC BiLSTM-PSO revision, Day 4 (May 31, 2026)
# -----------------------------------------------------------------------------
# Addresses reviewer comments:
#   12  -> Phase D: Welch's t + Benjamini-Hochberg FDR + Cohen's d + Cliff's
#                   delta on the 7 climate predictors (fire vs non-fire)
#   15b -> Phase E: Theil-Sen slope + Mann-Kendall trend test replacing OLS
#                   on Fig 5a annual burned-area time series
#   16  -> Phase F: Fig 11 redraw with bootstrap 95% CI envelope overlay
#   1   -> Phase G: BC Regional District fire-density ranking + choropleth
#                   (policy-actionable translation of the susceptibility map)
#
# Run inside the (wildfire) conda environment:
#   conda activate wildfire
#   cd C:\Users\saadz\Documents\wildfire-bc-bilstm-pso
#   python src\phase_d_to_g.py
#
# Tkinter file pickers will open in sequence. Cancel any phase you don't
# want to run by hitting Cancel on its picker.
# -----------------------------------------------------------------------------
...
matplotlib.use("Agg")  # headless backend - no Qt/Tk needed for plotting
...
def cohens_d(x: np.ndarray, y: np.ndarray) -> float:
    """Cohen's d with pooled SD (Hedges' bias correction not applied)."""
...
def cliffs_delta(x: np.ndarray, y: np.ndarray) -> float:
    ...
    if len(x) == 0 or len(y) == 0:
        return np.nan
    # Vectorised Mann-Whitney-like comparison
    nx, ny = len(x), len(y)
    # For large n, use rank-based formula to avoid O(nx*ny) memory.
    if nx * ny > 5_000_000:
        # U = sum of ranks of x in combined sample - nx*(nx+1)/2
...
def benjamini_hochberg(pvals: np.ndarray, alpha: float = 0.05):
    """Return (q_values, reject_mask) for Benjamini-Hochberg FDR."""
...
# -----------------------------------------------------------------------------
# Phase D - FDR + effect sizes for 7 climate predictors
# -----------------------------------------------------------------------------
CLIMATE_CANDIDATES = [
...
def phase_d(project_root: Path) -> dict:
    ...
    if not climate_cols:
        # Fall back to any numeric column shared by both
        climate_cols = [
            c for c in common
    ...
    # Sort by absolute Cliff's delta (effect size)
...
# -----------------------------------------------------------------------------
# Phase E - Theil-Sen + Mann-Kendall on Fig 5a (annual burned area)
# -----------------------------------------------------------------------------
def phase_e(project_root: Path) -> dict:
    ...
    if not ts_file:
        # Try to build from fire shapefile if user cancels
    ...
    # Mann-Kendall (Hamed-Rao modification not used; vanilla MK)
    try:
    ...
    # OLS line (light gray, for comparison)
...
# -----------------------------------------------------------------------------
# Phase F - Fig 11 with bootstrap 95% CI envelope overlay
# -----------------------------------------------------------------------------
def phase_f(project_root: Path) -> dict:
    ...
    # Bootstrap CI envelopes - resample with replacement, n_iter times
    n_iter = 1000
    ...
    for i in range(n_iter):
        ...
        # PR curve has descending recall - sort ascending for interp
        order = np.argsort(r)
...
# -----------------------------------------------------------------------------
# Phase G - BC Regional District fire-density ranking
# -----------------------------------------------------------------------------
def phase_g(project_root: Path) -> dict:
    ...
    # Project both to BC Albers (EPSG:3005) for accurate area in km^2
    ...
    # Identify the name column on the RD shapefile
    ...
    if name_col is None:
        # fall back to first object-typed column
        for c in rd.columns:
    ...
    # Aggregate fire size if available
