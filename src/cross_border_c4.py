# -*- coding: utf-8 -*-
"""
cross_border_c4.py  --  Reviewer comment #4, corrected Tier 1 + Tier 3.

Runs standalone from PowerShell (NOT the ArcGIS Python window). Needs the same
stack your last run used: rasterio, geopandas, shapely, scipy, numpy, matplotlib.

WHAT CHANGED vs the hex run that gave weak/garbled numbers:
  * TIER 1 (cross-border rank consistency): pairs BC susceptibility with US burn
    probability using square STRADDLE TILES (each tile contains cells on both
    sides of the border -> one mean-BC and one mean-US value per tile), then
    Spearman rho across tiles. Reported at a primary tile size AND swept across
    several sizes, so the estimate is shown to be robust rather than an artifact
    of one arbitrary tiling. Smaller tiles = more paired units = more power; the
    sweep makes the autocorrelation/power tradeoff explicit.
    (Spearman is rank-based, so it is invariant to the BC-vs-US scale difference.)
  * TIER 3 (no artificial boundary edge): the earlier raw-value Mann-Whitney
    "BC near vs US near" was meaningless -- the two products live on different
    scales (BC ~0-1, BP ~0-0.06) so they NEVER overlap and you always get
    "perfect separation". Replaced with the defensible question: does EITHER
    product show an anomaly RIGHT AT the border vs deeper in its own strip?
    -> within-product Mann-Whitney (near-border band vs the rest), same product,
       same scale, valid test. No artificial edge => near ~ far.

OUTPUTS (into your revision folder):
  03_Figures/Fig_S6b_xborder_consistency_v2.{pdf,png}
  06_Final_Tables/T_C4_tier1_tier3_v2.json
  06_Final_Tables/T_C4_tile_pairs_v2.csv

RUN (PowerShell), from anywhere:
  python cross_border_c4.py --bc-susc "C:/path/to/your_susceptibility.tif"
If you omit --bc-susc the script lists candidate .tif files and exits.
"""

import os, sys, json, csv, glob, argparse

# ----- dependency check with a clear message -----
try:
    import numpy as np
    import rasterio
...
# ----------------------------------------------------------------------
# CONFIG (defaults match your project layout; override via CLI where noted)
# ----------------------------------------------------------------------
...
TARGET_EPSG = 3005          # BC Albers
CELL        = 1500          # 1.5 km grid
N_BINS      = 5             # quantile classes for weighted kappa
MIN_CELLS   = 3            # min valid cells per side for a tile to count
NEAR_M      = 5000          # Tier-3 near-border band (m each side)
TILE_PRIMARY_KM = 20        # primary straddle-tile size
...
def build_grid(strip_paths):
    """Common 1.5 km grid in TARGET_EPSG covering the union of the strips."""
    xs, ys = [], []
...
def quantile_class(vals, k):
    ...
    return np.digitize(vals, qs)  # 0..k-1
...
def tile_pairs(bc, us, bc_m, us_m, transform, tile_km):
    """One (mean BC, mean US) per square tile that straddles the border."""
...
def main():
    ...
    # weighted kappa at the primary tile size
    kap = float("nan")
