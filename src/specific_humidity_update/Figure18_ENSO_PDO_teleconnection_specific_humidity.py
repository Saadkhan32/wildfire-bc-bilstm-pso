"""
Regenerate Fig. 18 - ENSO/PDO teleconnection controls on BC wildfire (2000-2024)
with the humidity predictor switched from relative humidity to SPECIFIC HUMIDITY.

Three panels:
  (a) ENSO -> fire-season climate -> area burned (paired dot-whisker, Spearman
      rho with Bonett-Wright Fisher-z 95% CI)
  (b) ENSO (ONI) and PDO vs fire-season climate (correlation heatmap)
  (c) Interannual vs trend control (|rho| with area burned, raw vs detrended)

This is a fully self-contained version of the original generator
(reviewer_response_C2_C15/code/make_figures.py :: fig_combined) - it has no
repo-internal imports, so reviewers can run it stand-alone.

Specific-humidity results (replacing the old relative-humidity values):
  ENSO (ONI summer) -> humidity : rho = +0.18 (n.s.)   [was -0.64 ***]
  humidity -> area burned       : rho = +0.36 (n.s.)   [was -0.81 ***]
  panel (c) raw / detrended     : 0.36 / 0.19          [was 0.61 / 0.55]
=> specific humidity is no longer a significant teleconnection mediator.

Input (repo-relative):
  reviewer_response_C2_C15/data/analysis_dataset.csv
    annual 2000-2024; must contain columns:
      Tmax, Precip, SH, Soil, PDSI, AET, area70,
      oni_djf, oni_mam, oni_jja, pdo_djf, pdo_mam, pdo_jja
  If the SH column is absent it is derived from
  data/BC_ERA5Land_monthly_specific_humidity_2000_2024.csv (fire-season mean).

Output: figs/Fig18_ENSO_teleconnection_specifichumidity.{pdf,tif,png}

Run from repo root:
  python src/specific_humidity_update/regenerate_fig18_teleconnection.py
...
DATA = os.path.join(REPO, "reviewer_response_C2_C15", "data", "analysis_dataset.csv")
SH_MONTHLY = os.path.join(REPO, "data", "BC_ERA5Land_monthly_specific_humidity_2000_2024.csv")
OUTDIR = os.path.join(REPO, "figs")
OUT = os.path.join(OUTDIR, "Fig18_ENSO_teleconnection_specifichumidity")
...
# ---- load data; ensure a specific-humidity column "SH" exists -------------
def load_data():
    ...
    if "SH" not in D.columns:
        ...
        fs = sh[sh.month.isin([5, 6, 7, 8])].groupby("year")["sh"].mean()  # May-Aug mean
        D["SH"] = D.index.map(fs)
    return D
...
def spearman_ci(D, a, b):
    """Spearman rho with Bonett-Wright Fisher-z 95% CI."""
    x, y = clean(D, a, b)
...
# climate variables: SH = specific humidity (was RH)
...
def panel_b(ax, D):
    ...
    ax.axhline(2.5, color="0.15", lw=1.9)   # ENSO | PDO separator
