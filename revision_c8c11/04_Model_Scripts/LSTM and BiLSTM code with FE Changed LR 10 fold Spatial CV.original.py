# -*- coding: utf-8 -*-
"""
Wildfire LSTM / BiLSTM (pure RNN — NO CNN) • Leakage-safe RF-RFE • 30 epochs • Loader sidecars
Writes per model type (LSTM/ BILSTM) into its own subfolder:
  - final_model.keras
  - static_preprocessor.joblib
  - selected_features_final.csv   (column: selected_feature)
  - feature_meta.json

Flat-input model (n_features,) internally reshaped to (T=n_features, 1) so your loader
can call model(X_sel) directly after pre+feature selection.

Requires:
  pip install numpy pandas scikit-learn tensorflow keras joblib tqdm matplotlib
"""
...
TEST_SIZE     = 0.30
RANDOM_STATE  = 42
GRID_KM       = 50.0       # meta only (for spatial grouping reference)
...
def compute_block_ids(lat: np.ndarray, lon: np.ndarray, grid_km: float = GRID_KM) -> np.ndarray:
    lat = np.asarray(lat, dtype=float)
    lon = np.asarray(lon, dtype=float)
    mean_lat = float(np.nanmean(lat)) if np.isfinite(np.nanmean(lat)) else 45.0
    m_per_deg_lon, m_per_deg_lat = meters_per_degree(mean_lat)
    x_m = (lon - np.nanmin(lon)) * m_per_deg_lon
    y_m = (lat - np.nanmin(lat)) * m_per_deg_lat
    cell_m = grid_km * 1000.0
    gx = np.floor(x_m / cell_m).astype(int)
    gy = np.floor(y_m / cell_m).astype(int)
    gy_off = gy - gy.min()
    return gx.astype(np.int64) * (gy_off.max() + 1) + gy_off.astype(np.int64)
...
def build_lstm_or_bilstm_flatinput(
    ...
    model.compile(optimizer=opt, loss=loss, metrics=[keras.metrics.AUC(name="auc", curve="ROC")])
    return model
...
def main():
    ...
    # spatial columns (excluded from training; written to meta)
    ...
    if lon_name and lat_name:
        ...
        X_tr_df, X_te_df, y_tr, y_te, idx_tr, idx_te = train_test_split(
            X, y, np.arange(len(y)), test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=y
        )
        _ = compute_block_ids(lat_all[idx_tr], lon_all[idx_tr], GRID_KM)
    else:
        X_tr_df, X_te_df, y_tr, y_te = train_test_split(
            X, y, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=y
        )

    # train each model family and save in its own folder
    for model_type in tqdm(RUNS, desc="[Main] Train & Save"):
        p = BASE_HPS[model_type]
        bidir = (model_type == "bilstm")
        model_dir = os.path.join(out_root, model_type.upper())
        os.makedirs(model_dir, exist_ok=True)

        # --- Preprocess + RFE on training ONLY (leakage-safe) ---
        pre = build_preprocessor(X_tr_df)
        pre.fit(X_tr_df)
        mask, feat_names = run_rfe(pre, X_tr_df, y_tr)

        # transform with same pre + mask
        Xtr = transform_with_mask(pre, X_tr_df, mask)
        Xte = transform_with_mask(pre, X_te_df, mask)

        # class weights
        cw = maybe_class_weights(y_tr)

        # internal val split for ES
        idx_tr_sub, idx_va_sub = train_test_split(
            np.arange(len(y_tr)), test_size=0.15, random_state=RANDOM_STATE, stratify=y_tr
        )

        # model (flat input; internal reshape)
        ...
        es = keras.callbacks.EarlyStopping(monitor="val_auc", mode="max",
                                           patience=EARLYSTOP_PATIENCE, restore_best_weights=True, verbose=0)
        rl = keras.callbacks.ReduceLROnPlateau(monitor="val_auc", mode="max",
                                               factor=0.5, patience=PLATEAU_PATIENCE, min_lr=MIN_LR, verbose=0)

        model.fit(
            Xtr[idx_tr_sub], y_tr[idx_tr_sub],
        ...
        # ---- SAVE: exactly what the loader expects in the SAME folder as .keras ----
        # 1) model
        model_path = os.path.join(model_dir, "final_model.keras")
        model.save(model_path)
        print(f"[OK] Saved model → {model_path}")

        # 2) static_preprocessor.joblib  (ONLY the fitted ColumnTransformer)
        pre_path = os.path.join(model_dir, "static_preprocessor.joblib")
        joblib.dump(pre, pre_path)
        print(f"[OK] Saved preprocessor → {pre_path}")

        # 3) selected_features_final.csv  (transformed names kept by RFE)
        sel_path = os.path.join(model_dir, "selected_features_final.csv")
        pd.DataFrame({"selected_feature": feat_names}).to_csv(sel_path, index=False)
        print(f"[OK] Saved selected features → {sel_path}")

        # 4) feature_meta.json (schema hints for loader)
        meta = {"used_cols": used_cols, "lat_col": lat_name, "lon_col": lon_name, "grid_km": GRID_KM}
        with open(os.path.join(model_dir, "feature_meta.json"), "w", encoding="utf-8") as f:
            json.dump(meta, f, indent=2)
        print(f"[OK] Saved feature_meta.json → {model_dir}")

        # quick holdout check (optional)
        y_prob = model.predict(Xte, batch_size=int(p["batch_size"]), verbose=0).ravel()
        auc = roc_auc_score(y_te, y_prob); ap = average_precision_score(y_te, y_prob)
        print(f"[HOLDOUT] {model_type.upper()}  AUC={auc:.4f}  AP={ap:.4f}")

    print("\n[DONE] Per-model folders ready for the loader:\n - LSTM/\n - BILSTM/\nPick the .keras file; sidecars are next to it.")
